...
function find_empty_loop(loop$tiles, loop$idx) {
  while (true) {
    ...
    if (tiles.atLeastLength(1) && tiles.head === 0) {
      ...
      return new Error("Error find element of 0 value in tiles list");
    }
  }
}
...
function init2(_) {
  return new Model2(generate_tiles(0, 15), 0);
}
function update_model_after_shuffle(new_data) {
  return new Model2(new_data[1], new_data[0]);
}
...
function update_model_with_move(idx, value, model) {
  return new Model2(
    swap_empty(model.tiles, model.empty_idx, [idx, value]),
    idx
  );
}
function handle_move(idx, value, model) {
  let $ = is_adjucent(idx, model.empty_idx);
  if ($) {
    return update_model_with_move(idx, value, model);
  } else {
    return model;
  }
}
function update(model, msg) {
  if (msg instanceof Shuffle) {
    ...
    let idx = msg[0];
    let value = msg[1];
    return handle_move(idx, value, model);
  }
}
function button_view(text3, event_msg) {
  return button(
