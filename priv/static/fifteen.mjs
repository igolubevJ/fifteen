...
function insert(dict2, key3, value3) {
  return map_insert(key3, value3, dict2);
}
...
function pop_grapheme(string4) {
  ...
  if (first4) {
    return new Ok2([first4, string4.slice(first4.length)]);
  } else {
...
function do_element_list_handlers(elements2, handlers2, key3) {
  return index_fold(
    ...
    (handlers3, element3, index4) => {
      let key$1 = key3 + "-" + to_string(index4);
...
var LustreClientApplication = class _LustreClientApplication {
  ...
  static start({ init: init3, update: update2, view: view2 }, selector, flags) {
    ...
    return new Ok2((action) => app.send(action));
  }
  /**
   * @param {Element} root
  ...
  #flush(effects = []) {
    ...
    while (effects.length > 0) {
      ...
      effect({ dispatch: dispatch2, emit: emit2, select: select3, root });
    }
...
function generate_tiles(from2, to) {
  return range(from2, to);
}
function is_adjucent(idx_a, idx_b) {
  let row_a = divideInt(idx_a, 4);
  let col_a = remainderInt(idx_a, 4);
  let row_b = divideInt(idx_b, 4);
  let col_b = remainderInt(idx_b, 4);
  let row_test = absolute_value(row_a - row_b) === 1 && col_a === col_b;
  let col_test = absolute_value(col_a - col_b) === 1 && row_a === row_b;
  return row_test || col_test;
}
function swap_empty(tiles, zero_idx, tile) {
  return index_map(
    tiles,
    (v, idx) => {
      if (idx === zero_idx) {
...
function find_empty_loop(loop$tiles, loop$idx) {
  while (true) {
    let tiles = loop$tiles;
    let idx = loop$idx;
    if (tiles.atLeastLength(1) && tiles.head === 0) {
      let el = tiles.head;
      return new Ok2(idx);
    } else if (tiles.atLeastLength(1)) {
      let rest = tiles.tail;
      loop$tiles = rest;
      loop$idx = idx + 1;
    } else {
      return new Error2("Error find element of 0 value in tiles list");
    }
  }
}
function find_empty(tiles) {
  let $ = find_empty_loop(tiles, 0);
  if (!$.isOk()) {
    throw makeError(
      "let_assert",
      "fifteen/utils",
...
function shuffled_tile(tiles) {
  let res = shuffle(tiles);
  return [find_empty(res), res];
}

// build/dev/javascript/fifteen/fifteen.mjs
var Model3 = class extends CustomType {
  constructor(tiles, empty_idx) {
    super();
    this.tiles = tiles;
    this.empty_idx = empty_idx;
  }
};
...
function init2(_) {
  return new Model3(generate_tiles(0, 15), 0);
}
function update_model_after_shuffle(new_data) {
  return new Model3(new_data[1], new_data[0]);
}
function handle_shuffle(model) {
  let _pipe = model.tiles;
  let _pipe$1 = shuffled_tile(_pipe);
  return update_model_after_shuffle(_pipe$1);
}
function update_model_with_move(idx, value3, model) {
  return new Model3(
    swap_empty(model.tiles, model.empty_idx, [idx, value3]),
    idx
  );
}
function handle_move(idx, value3, model) {
  let $ = is_adjucent(idx, model.empty_idx);
  if ($) {
    return update_model_with_move(idx, value3, model);
  } else {
    return model;
  }
}
function update(model, msg) {
  if (msg instanceof Shuffle) {
    return handle_shuffle(model);
  } else {
    let idx = msg[0];
    let value3 = msg[1];
    return handle_move(idx, value3, model);
  }
}
function button_view(text5, event_msg) {
  return button(
    toList([class$("shuffle-btn"), on_click(event_msg)]),
    toList([text2(text5)])
  );
}
var tile_size = 100;
var tile_gap = 6;
function generate_tile_style(tile_idx) {
  return style(
    toList([
      [
        "top",
        to_string(divideInt(tile_idx, 4) * tile_size + tile_gap) + "px"
      ],
      [
        "left",
        to_string(remainderInt(tile_idx, 4) * tile_size + tile_gap) + "px"
      ]
    ])
  );
}
function empty_tile_view(tile_idx) {
  return div(
    toList([class$("tile empty"), generate_tile_style(tile_idx)]),
    toList([text2("")])
  );
}
function tile_view(tile_idx, tile, event_msg) {
  return div(
    toList([
      class$("tile"),
      generate_tile_style(tile_idx),
      on_click(event_msg)
    ]),
    toList([text2(to_string(tile))])
  );
}
function tiles_view(model) {
  return index_map(
    model.tiles,
    (tile, idx) => {
      if (tile === 0) {
        return empty_tile_view(idx);
      } else {
        return tile_view(idx, tile, new Move(idx, tile));
      }
    }
  );
}
var logo_url = "/priv/static/image/logo.png";
function view(model) {
  return div(
    toList([]),
    toList([
      div(
        ...
        toList([img(toList([src(logo_url), id("logo")]))])
      ),
      div(
        toList([class$("centered")]),
        toList([
          div(toList([class$("game-container")]), tiles_view(model))
        ])
      ),
      div(
        toList([class$("centered")]),
        toList([button_view("Shuffle", new Shuffle())])
      )
    ])
  );
}
